...
// date1 : 기준 날짜
const getDiffDays = (date1, date2, includeToday = false) => {
  const { year: y1, month: m1, day: d1 } = date1;
  const { year: y2, month: m2, day: d2 } = date2;

  const result = DateTime.local(y1, m1, d1).diff(
    DateTime.local(y2, m2, d2),
    "days"
  ).days;

  if (result < 0) {
    if (includeToday) return -result + 1;
  }
  return -result;
};
...
export { getDiffDays, years, months, days, validDate };
